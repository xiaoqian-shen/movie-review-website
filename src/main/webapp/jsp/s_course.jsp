  ...
  try {
    ...
    if (conn != null) {
      ...
      while(rs.next()){
        ...
        pStmt = conn.prepareStatement("select course_id,score from takes where student_id='"+username+"' and course_id='"+course.course_id+"'");
        ResultSet rss = pStmt.executeQuery();
        if(rss.next()) {
          if(rss.getString("score")==null)
            course.flag=1;
          else
            course.flag = 2;
        }
        ...
        <tr>
          ...
          <%
            if(course.flag==0){
              String action="take";
          %>
          <td><div class="button-group"> <a class="button border-blue" href="dotake.jsp?s_id=<%=username%>&c_id=<%=course.course_id%>&action=<%=action%>"><span class="icon-cog"></span>选课</a> </div></td>
          <%
            }
            else if(course.flag==1){
              String action="cancel";
          %>
            <td><div class="button-group"> <a class="button border-red" href="dotake.jsp?s_id=<%=username%>&c_id=<%=course.course_id%>&action=<%=action%>"><span class="icon-trash-o"></span>删除</a> </div></td>
          <%
            }
            else{
          %>
